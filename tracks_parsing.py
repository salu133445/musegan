...
def save_flat_piano_roll(piano_roll, midi_dir, prefix, msd_id, midi_md5):
    filepath = os.path.join(midi_dir, prefix + '.'+ FILETYPE)
...
def check_dir(msd_id, midi_md5):
    dir_to_check = os.path.join(RESULT_PATH, msd_id_to_dirs(msd_id), midi_md5)
...
if __name__ == "__main__":
    ...
    for song_idx in range(len(subset_id_list)):
        ...
        for midi_md5 in midi_dict[msd_id]:
            ...
            midi_dir = check_dir(msd_id, midi_md5)
            ...
            for idx in range(5):
                ...
                save_flat_piano_roll(piano_roll, midi_dir, prefix[idx], msd_id, midi_md5)
            
            counter += 1
            # print('%d/%d' %( counter, song_idx), 'collected!', instrus_act)
